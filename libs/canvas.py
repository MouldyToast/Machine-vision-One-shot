...
class Canvas(QWidget):
    ...
    def mouseMoveEvent(self, ev):
        ...
        if Qt.MouseButton.LeftButton & ev.buttons():
            if self.selected_vertex():
                ...
                self.scrollRequest.emit(delta_pos.x(), Qt.Orientation.Horizontal)
                self.scrollRequest.emit(delta_pos.y(), Qt.Orientation.Vertical)
                self.update()
            return
    ...
    def wheelEvent(self, ev):
        ...
        mods = ev.modifiers()
        if mods == (Qt.KeyboardModifier.ControlModifier | Qt.KeyboardModifier.ShiftModifier) and v_delta:
